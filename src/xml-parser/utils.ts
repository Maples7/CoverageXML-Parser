...
export function updateSourceFilePath(cachePath: vscode.Uri, oldPath: string, newPath: string) {

      let offset: number = 1
      const offsetUpperBound = Math.min(oldPath.length, newPath.length)
      while (offset < offsetUpperBound && oldPath[oldPath.length - offset] === newPath[newPath.length - offset]) { offset++ }
      ...
      const sourceFileMap = vscode.Uri.joinPath(cachePath, 'SourceFileMap.json').path
